<tr class="text-left text-slate-900">
    <td class="pl-6 py-4 pr-3 font-medium">{{ $player->first_name }}</td>
    <td class="pl-4 py-4 text-left text-slate-500 hidden sm:table-cell">{{ $player->last_name }}</td>
    <td class="pl-4 py-4 text-left text-slate-500 hidden sm:table-cell">{{ $player->grad_year }}</td>
    <td class="pl-4 py-4 text-left text-slate-500 hidden sm:table-cell">{{ $player->active }}</td>
                ...
                <x-dialog wire:model="showEditDialog">
                    ...
                    <x-dialog.panel>
                        <form wire:submit="save" class="flex flex-col gap-4">
                            <h2 class="text-3xl font-bold mb-1">Edit your roster</h2>
                            ...
                            <label class="flex flex-col gap-2">
                                Content
                                <textarea wire:model="form.last_name" class="px-3 py-2 border font-normal rounded-lg border-slate-300 read-only:opacity-50 read-only:cursor-not-allowed"></textarea>
                                @error('form.last_name')<div class="text-sm text-red-500 font-normal">{{ $message }}</div>@enderror
                            </label>

                            <label class="flex flex-col gap-2">
                                Content
                                <textarea wire:model="form.grad_year" class="px-3 py-2 border font-normal rounded-lg border-slate-300 read-only:opacity-50 read-only:cursor-not-allowed"></textarea>
